...
# Create txt file
file = open('testing_comp_mask.txt', 'w')

image_name = "GBI_CONTOURS_RES3.jpg"
...
# bitwise_not because background is black and is supposed to be white
notGray = cv2.bitwise_not(gray)

# Morphological operation to remove more noise
notGray = cv2.morphologyEx(notGray, cv2.MORPH_CLOSE, np.ones((7,7),np.uint8))

# Threshold value changes depending on testing image vs real image
ret, thresh = cv2.threshold(notGray, 240, 255, cv2.THRESH_BINARY)
